...
def load_student_lists(classifier_path: str):
    """Load enrolled students directly from the trained classifier."""
    import pickle

    with open(classifier_path, "rb") as f:
        classifier_data = pickle.load(f)

    names_map = classifier_data["label_to_name"]

    indices = sorted(names_map.keys())
    names = [names_map[i] for i in indices]

    # Current system is name-based.
    rolls = [str(i) for i in indices]
...
def process_session(recognizer: AttendanceRecognizer,
    ...
    for s in sorted(result.present_students, key=lambda x: x['name']):
        print(f"  ✓ {s['name']:<25} "
          f"conf={s['avg_confidence']:.2f}  seen in {s['images_seen']}/{s['total_images']} images")

    print(f"\nAbsent ({len(result.absent_students)}):")
    for s in sorted(result.absent_students, key=lambda x: x['name']):
        print(f"  ✗ {s['name']}")
...
def main():
    ...
    all_names, _ = load_student_lists(PATHS["classifier"])
    ...
    if args.session:
        ...
        if not session_path.exists():
            ...
            sys.exit(1)
        process_session(recognizer, session_path, all_names, PATHS["output_dir"])
        ...
        for session_dir in session_dirs:
            process_session(recognizer, session_dir, all_names, PATHS["output_dir"])
